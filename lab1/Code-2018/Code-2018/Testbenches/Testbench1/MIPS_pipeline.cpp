    ...
    while (1) {
        ...
        if(!state.MEM.nop){
            ...
            if(!state.MEM.rd_mem && !state.MEM.wrt_mem)// R type
                newState.WB.Wrt_data = state.MEM.ALUresult;// 写回寄存器
        ...
        if(!state.EX.nop){
            ...
            ALUin1 = state.EX.Read_data1;//myRF.ReadData1;
            ALUin2 = (state.EX.is_I_type)?signextend(state.EX.Imm):state.EX.Read_data2;//(IType.to_ulong())?signext:myRF.ReadData2;
        ...
        if(!state.IF.nop){
            if(flag)
                ...
                instruction = myInsMem.readInstr(newState.IF.PC);
                //flag=1;
            }
