...
int main()
{
    ...
    state.IF.PC=0;
    state.IF.nop=false;
    //state.IF.flag=1;// if it is a branch flag = 0;
    state.ID.nop=true;
    state.EX.nop=true;
    state.MEM.nop=true;
    state.WB.nop=true;
    ...
    while (1) {
        ...
        /* --------------------- WB stage --------------------- */
        if(!state.WB.nop){
            //cout<<"wrt_enable: "<<state.WB.wrt_enable<<endl;
        ...
        if(!state.MEM.nop){
            ...
            //cout<<"WBwrt_reg_addr: "<<newState.WB.Wrt_reg_addr<<endl;

            if(!state.MEM.rd_mem && !state.MEM.wrt_mem)// R type
            {
            ...
            // 写回寄存器
                ...
                //Mem to Mem
                //std::cout << "Wrt Mem Writ Address: "<<state.MEM.ALUresult <<" Store Data: " <<state.MEM.Store_data<< std::endl;
                if(state.WB.Wrt_reg_addr == state.MEM.Rt && state.WB.wrt_enable){
                    state.MEM.Store_data = state.WB.Wrt_data;
                }
                //std::cout << "After Forward>>>Wrt Mem Writ Address: "<<state.MEM.ALUresult <<" Store Data: " <<state.MEM.Store_data<< std::endl;*/
                myDataMem.writeDataMem(state.MEM.ALUresult,state.MEM.Store_data);
            }
            //cout<<"AFTERWBwrt_reg_addr: "<<newState.WB.Wrt_reg_addr<<endl;
        }
        else{
        ...
        if(!state.EX.nop){
            ...
            //cout<<cycle<<endl;
            //cout<<"MEMALUsult: "<<newState.MEM.ALUresult<<endl;
            //cout<<"MEMwrt_enable: "<<newState.MEM.wrt_enable<<endl;
            //cout<<"MEMwrt_reg_addr: "<<newState.MEM.Wrt_reg_addr<<endl;
            if(state.EX.wrt_mem)
        ...
        if(!state.ID.nop){
            ...
            newState.EX.alu_op = (RType==1 && (funct.to_ulong()==35))?0:1;//判断是不是subu subu是0 其他是1
            //cout<<"aluop: "<<newState.EX.alu_op<<endl;
